...
// Every generated page carries this: the whole vault is deleted and rebuilt
// from scratch on every run (see the full-regenerate note below), so a
// hand-edit made directly in Obsidian is silently lost on the next export
// with no other warning anywhere. This doesn't fix that — it just stops it
// from being a silent trap.
const GENERATED_NOTE =
  '_Auto-generated by memory-mcp — this file is fully rebuilt every run; edits made here will be overwritten._';
    ...
    .action(async (opts: { out: string }) => {
      ...
      try {
        ...
        for (const [category, list] of byCategory) {
          const lines = [`# ${category}`, '', GENERATED_NOTE, ''];
        ...
        for (const day of allDays) {
          const lines = [`# ${day}`, '', GENERATED_NOTE, ''];
